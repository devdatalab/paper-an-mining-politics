...
binscatter winner_any_crim pshock, control(base_value $con_controls) absorb(sygroup) xtitle("Price Shock") ytitle("Probability Criminal is Elected")
...
binscatter ln_diff_net_assets  pshock if winner1 == 1, absorb(sygroup) control(base_value ln_net_assets1 $con_controls ) xtitle("Price Shock") ytitle("Log Asset Change")
graphout bin_mh_assets

binscatter more_crime  pshock if winner1 == 1, absorb(sygroup) control(base_value ln_crim1 $con_controls ) xtitle("Price Shock") ytitle("Probability MLA Accumulates Additional Charges")
